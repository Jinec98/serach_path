...
void SerachPath::createToolBar()
{
	...
	connect(generateMazeAction, &QAction::triggered, this, &SerachPath::generateMazeActionSlot);
	...
	generateMazeTool = addToolBar("Generate Maze");
	generateMazeTool->addAction(generateMazeAction);
...
void SerachPath::helpActionSlot()
{
	...
	infoText += "2、点击“生成迷宫”\t选择需要自动生成的迷宫规模\n";
	infoText += "\t本程序应用DFS算法自动生成存在唯一路径的迷宫，自定义迷宫规模应在49*49以内，选择目录保存文件即可。";
	infoText += "保存的文件为本程序可识别的txt文本，可用于后续“打开文件”使用。\n\n";
	infoText += "3、点击“打开文件”\t打开“.txt”文本文件\n";
	infoText += "\t该文件内容的格式为：第一行：两个数字m、n，分别表示迷宫的行与列。";
	infoText += "余下m行：每行n个字符，表示迷宫中坐标为（m，n）的地方的状态。其中：“ ”（一个空格）表示可行路径、“X”（大写英文字母X）表示墙（不可行走）、“S”表示起点、“D”表示终点。";
	infoText += "注意：迷宫的最适规模为5*5-30*30，在此情况下程序显示的最为清晰。迷宫最大规模不可超过50*50。\n\n";
